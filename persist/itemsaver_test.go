...
func TestItemSave(t *testing.T) {
	...
	resp, err := client.Get().Index("douban").Id(id).Do(context.Background())
	if err != nil {
		panic(err)
	}
	...
	if actual != bookTestData {
		log.Printf("excepted %+v but got %+v", bookTestData, actual)
	}

	_, err = client.Delete().Index("douban").Id(id).Do(context.Background())
	if err != nil {
		panic(err)
	}
}
