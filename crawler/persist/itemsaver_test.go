...
func TestItemSave(t *testing.T) {
	...
	resp, err := client.Get().Index("douban").Id(bookTestData.Id).Do(context.Background())
	if err != nil {
		panic(err)
	}
	...
	actualBook, _ := model.FromJsonObj(actual.Payload)
	actual.Payload = actualBook

	if actual != bookTestData {
		t.Errorf("excepted %+v\nbut got %+v", bookTestData, actual)
	}

	_, err = client.Delete().Index("douban").Id(bookTestData.Id).Do(context.Background())
	if err != nil {
		panic(err)
	}
}
