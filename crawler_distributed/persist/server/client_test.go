...
func TestItemSaver(t *testing.T) {
	...
	const host = config.ItemSaverPort
	go serveRpc(fmt.Sprintf(":%d", config.ItemSaverPort))

	time.Sleep(time.Second * 2)

	client, err := rpcsupport.NewClient(fmt.Sprintf(":%d", config.ItemSaverPort))
	if err != nil {
		panic(err)
	}
	result := ""
	err = client.Call(config.ItemSaverRpc, bookTestData, &result)
